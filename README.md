...
| 平台 | 架构 | 文件 | 说明 |
|------|------|------|------|
| Windows | x64 | `openclaw-config-*-windows-amd64.exe` | 直接运行 |
| Linux | x64 | `openclaw-config-*-linux-amd64` | 需添加执行权限 |
| Linux | ARM64 | `openclaw-config-*-linux-arm64` | 需添加执行权限 |
| macOS | Universal | `openclaw-config-*-macos-universal` | 支持 Intel 和 Apple Silicon |
...
下载后直接双击运行 `openclaw-config-*-windows-amd64.exe`
...
1. 下载后添加执行权限：`chmod +x openclaw-config-*-macos-universal`
2. 双击运行或在终端执行
...
- **CNB**：推送 Tag 时自动构建 Linux (AMD64/ARM64) 和 Windows 版本
- **GitHub Actions**：推送 Tag 时自动构建 Linux、Windows 和 macOS 版本

两者互补，确保所有平台都有自动构建的版本。
