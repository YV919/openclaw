...
// readLineRaw 在 Windows 控制台用 ReadConsoleInputW 逐字符读取一行，支持 ESC 取消与退格。
// masked=true 时回显 * 而非字符（用于密钥）。返回 (文本, 是否被 ESC 取消)。
// 与菜单同源（ReadConsoleInputW 不做行编辑/回显），故手动回显不会重复。
func readLineRaw(masked bool) (string, bool) {
	...
	for {
		...
		if rec.EventType != keyEvent || rec.BKeyDown == 0 {
			continue
		}
		ch := rec.UnicodeChar
		switch {
		case rec.WVirtualKeyCode == vkReturn || ch == '\r' || ch == '\n':
			fmt.Print("\r\n")
			return string(runes), false
		case rec.WVirtualKeyCode == vkEscape || ch == 27:
			return "", true
		case ch == 3: // Ctrl+C
			restoreConsole()
			fmt.Println()
			os.Exit(130)
		case rec.WVirtualKeyCode == vkBack || ch == 8 || ch == 127:
			if len(runes) > 0 {
...
// 保留以维持与参考工具一致的平台接口；本工具不写环境变量，但 Windows 平台符号需对称。
